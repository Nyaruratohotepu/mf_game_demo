...
namespace Assets.Scripts.Weapons
{
    ...
    //双手武器动作类
    public class TwoHandGunAnimation : IWeaponAnimation
    {
        ...
        //构造函数依赖注入
        public TwoHandGunAnimation(GameObject gunObject, Animator ownerAnimator)
        {
            gunObj = gunObject;
            leftHandIK = gunObj.transform.Find("LeftHandIK");
            this.ownerAnimator = ownerAnimator;
            //将动画设定为双手武器的动画控制器
            RuntimeAnimatorController ac = (RuntimeAnimatorController)Resources.Load(defaultAnimatorPath);
            if (ac != null)
                ownerAnimator.runtimeAnimatorController = ac;
